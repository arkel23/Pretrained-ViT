...
def standard_transform(args, is_train):
    ...
    if 'cifar' in args.dataset_name and args.image_size == 32:
        aa = CIFAR10Policy()
    elif args.dataset_name == 'svhn' and args.image_size == 32:
        aa = SVHNPolicy()
    else:
        aa = ImageNetPolicy()
    ...
    if is_train:
        ...
        if args.blur > 0:
            t.append(transforms.RandomApply(
                [transforms.GaussianBlur(
                    kernel_size=args.image_size//20*2+1, sigma=(0.1, 2.0))], p=args.blur))
        ...
        if ((args.dataset_name in ['cifar10', 'cifar100', 'svhn'] and args.image_size == 32)
           or (args.dataset_name == 'tinyin' and args.image_size == 64)):
            t.append(transforms.Resize(args.image_size))
        else:
            t.append(transforms.Resize(
                (args.test_resize_size, args.test_resize_size),
                interpolation=transforms.InterpolationMode.BICUBIC))
            t.append(transforms.CenterCrop(args.image_size))
